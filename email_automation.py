...
# def display_info(df, email_idx, header):
...
#             if cont.lower() == 'y':
#                 for e_idx in email_idxs:
#                     display_info(df, e_idx, ut.cc_header)
...
#                 elif(action == "4"):
#                     for e_idx in email_idxs:
#                         display_info(df, e_idx, ut.status_header)
#             action = ut.input_prompt(ut.email_acton_msg)
...
def main():
    ...
    tracker = Tracker(in_file_dir)
    action = ut.input_prompt(ut.action_msg)

    switcher = {"1":tracker.prefix_migrate,
                "2":tracker.email_actions}
